...
project id `<your-plane-project-id>`, http://<your-plane-host>).
...
**Build agent: Fable 5 builds ALL tasks directly (the operator's directive 2026-07-03
...
| M0.2 | Tauri v2 shell: tray, global hotkey, sidecar spawn | medium | **done 2026-07-03** — tray toggle/quit, ctrl+shift+v, sidecar spawn/reap, AMD GL pin | M1.3 | hotkey delivery on Wayland pending the operator keypress check |
...
| M3.4 | Node click → markdown side panel | easy | code done — awaiting the operator click-test (read_note sandboxed to vault) | M3.1 | |
...
| M4.2 | Wire writes to obsidian-local-rest-api + cyanheads MCP | medium | **done 2026-07-03** — create+PATCH e2e via HTTPS 27124; write → watcher → graph node verified | M4.1 | plugin 4.1.3 installed (the operator-approved), key in .env |
...
| M5.3 | Local-first paid-API fallback + savings metric | medium | **done 2026-07-04 — LIVE-PROVEN**: worker down → paid-api lane, ledger 22 paid tokens, reason logged | M5.2 | ⚠ burns paid credits — confirm with the operator before load-testing |
...
| X1 | gpu-deck: /workflows-style mission control (HUD panel + window + vault-top TUI) | medium | **done 2026-07-04** — live-verified: per-GPU truth, ollama residency, running tasks w/ Plane chain, history, light controls | M6.1, M6.2 | the operator-requested 2026-07-04; spec via Q&A |
| X2 | gpu-deck: live tok/s + 1h averages per lane + cloud-orchestrator relay/monitoring | medium | **done 2026-07-04** — worker lanes via /metrics+/slots (verified 22 tok/s live); Hermes repointed through :11500 relay, per-model cloud lane awaiting first organic traffic | X1 | relay standalone (vault-cloud-proxy.service, always-on); rollback = restore ~/.hermes/config.yaml.bak-2026-07-04-pre-relay |
| X3 | vault TUI daily driver: deck + command deck (Textual; per TUI-DECISION-2026-07-04) | medium | **done 2026-07-04** — WS-pushed transcript, /run /hermes /ask /cancel prompt, esc-interrupt, e2e pilot-tested; Tauri HUD demoted to optional eye-candy | X1, X2 | research: RESEARCH-2026-07-04-vault-tui.md |
| X4 | Dump local ollama: llama.cpp+HF everywhere (llm-models canonical, embed worker, /pull, scout rewrite) | hard | **phase 1 done 2026-07-05** — workers+RAG+catalog off ollama; new agentic models downloading; decommission staged for the operator sudo | X1, M5.1 | rollback: re-enable ollama units until stores deleted |
